...
from config.column_mapping import ASSESSMENT_COLUMN_MAPPING
...
def load_assessments_data():
    try:
        ...
        moved_files(file_name="accessments.csv", is_processed=True)
        logger.info("loading assessment data done successfully")
    except Exception:
        db_connection.rollback()
        # moved_files(file_name="accessments.csv", is_processed=False)
